...
public class BTreeNode
{
	...
	public BTreeNode(byte[] bytes, int degree)
	{
		...
		if (bytes.length != getByteSize(degree))
		{
			throw new IllegalArgumentException(
					"byte array must be the amount of bytes required for a BTreeNode of specified degree");
		}
	 ...
	 * 
	 * @param pointer
	 *            pointer for the parent to be set to
	 */
	 ...
	 *             if index < 0 or >= #children
	 * @param index
	 *            index of child pointer
	 * @param value
	 *            value of child pointer
	 */
	...
	/**
	 * @param index
	 *            index of the child pointer to be returned
	...
	/**
	 * Adds a tree object and child pointer to the node. child pointer is added to
	 * right of index
	 * 
	...
	/**
	 * Searches node for an object and returns a result. SearchResult contains two
	 * values, a boolean wasFound, and int location location represents different
	 * values depending on the state of wasFound if wasFound = true: location in the
	 * index of the object in the node if wasFound = false: location is the pointer
	 * to the correct child to continue the BTree search location can also be -1,
	 * meaning the node is a leaf and the object was not found
	 *
	 * @param obj
	...
	public SearchResult searchNode(TreeObject obj)
	{
		...
		{ // object was not found
			if (isLeafNode)
			{ // no children, so return nothing found
	...
	/**
	 * returns the index where the object is in the node. -1 if the object is not
	 * present
	 * 
	 * @param obj
	...
	/**
	 * given a treeObject in the node, this method returns the index of the child to
	 * the object direct right
	 * 
	 * @param index
	 *            index of object
	 ...
	 * @return how many bytes a node in a tree of specified degree will take on
	 *         storage
	 */
	...
	public String toString()
	{
		...
		sb.append("#keys=" + objects.size() + " isLeaf=" + isLeafNode + " location=" + location + "\n");
	...
	/**
	 * Returned from the search method. The value of location represents two
	 * different things depending on the state of wasFound
	 *
	 * wasFound = true: location represents searched objects index in the current
	 * node wasFound = false: location represents the pointer to the proper child to
	 * continue the search location can also be -1, meaning the node is a leaf and
